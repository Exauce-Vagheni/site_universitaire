		...
		if (isset($_FILES['image']) AND !empty($_FILES['image']['name']) AND !empty(isset($_POST['auteur'])) AND !empty(isset($_POST['contenu']))) {
				...
				if ($_FILES['image']['size']<=$tailleMax) {
					$extensionUpload=strtolower(substr(strrchr($_FILES['image']['name'], '.'), 1));				
						...
						$chemin = '../../vue/img/infos_eglise/'.$image.'.'.$extensionUpload;
						$resultat=move_uploaded_file($_FILES['image']['tmp_name'],$chemin);
						if($resultat)
						{
							$insert=$connexion->prepare('INSERT INTO infos_eglise(auteur,titre,contenu,photo,date_pub) VALUES(:auteur,:titre,:contenu,:photo,NOW())');
							...
							header('location:../../vue/admin/ajout_pub_eglise.php');
						}
						else
						{
